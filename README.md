...
- 结果页/分享图：
  ...
  - 结算页下方提示文案为“长按图片保存”；点击无交互，用户可长按海报区域或下方提示区域保存图片；
  - 修复生产构建错误：`SharePosterCanvas` 中 `next/image` 导入名与浏览器 `new Image()` 冲突，现已改为 `NextImage` 别名避免类型报错；
...
- 前端联调：
  ...
  - `result` 页展示手写风结算海报图片；下方显示“长按图片保存”，点击无交互
  - `result` 页左上角提供返回主页按钮
